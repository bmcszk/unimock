...
type UniConfig struct {
	...
	// BaseDir is the directory containing the configuration file, used for resolving fixture paths
	BaseDir string `yaml:"-" json:"-"`

	// fixtureResolver handles loading fixture files, initialized when config is loaded
...
// Accepts an optional fixture resolver for loading fixture files
func (sf *ScenarioConfig) ToModelScenario(resolver *FixtureResolver) model.Scenario {
	// Set defaults
	...
	// Resolve data - if resolver is provided and data is a file reference, load it
	data := sf.Data
	if resolver != nil {
		resolvedData, err := resolver.ResolveFixture(sf.Data)
		if err == nil {
			data = resolvedData
		} else if strings.HasPrefix(sf.Data, "@") {
			// If data is a file reference but resolution failed, return empty data
			// This prevents leaking file paths in responses when fixtures are missing
			data = ""
		}
		// If fixture resolution fails for inline data (non-@ prefix), keep original data
	}
...
func LoadFromYAML(path string) (*UniConfig, error) {
	...
	// Get the base directory for resolving fixture paths
	...
	if unifiedErr == nil && (len(config.Sections) > 0 || len(config.Scenarios) > 0) {
		...
		config.fixtureResolver = NewFixtureResolver(baseDir)
		config.Normalize()
		return config, nil
	}
...
// GetFixtureResolver returns the fixture resolver for this config
